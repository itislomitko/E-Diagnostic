...
if ($mod=='login'){
    ...
    if($row){
        ...
        print_msg("Salah kobobotinasi username dan password.");
    }          
...
/** RELASI TAbobotAH */ 
    ...
    $kobobotinasi_ada = $db->get_row("SELECT * FROM tb_relasi WHERE kode_diagnosa='$kode_diagnosa' AND kode_gejala='$kode_gejala'");
    
    if($kode_diagnosa=='' || $kode_gejala=='' || $bobot=='' || $keterangan=='')
        print_msg("Field bertanda * tidak boleh kosong!");
    elseif($kobobotinasi_ada)
        print_msg("Kobobotinasi diagnosa dan gejala sudah ada!");
    else{
        $db->query("INSERT INTO tb_relasi (kode_diagnosa, kode_gejala, bobot, keterangan) VALUES ('$kode_diagnosa', '$kode_gejala', '$bobot', '$keterangan')");
    ...
    $kobobotinasi_ada = $db->get_row("SELECT * FROM tb_relasi WHERE kode_diagnosa='$kode_diagnosa' AND kode_gejala='$kode_gejala' AND ID<>'$_GET[ID]'");
    ...
    elseif($kobobotinasi_ada)
        ...
        $db->query("UPDATE tb_relasi SET kode_diagnosa='$kode_diagnosa', kode_gejala='$kode_gejala', bobot='$bobot', keterangan='$keterangan' WHERE ID='$_GET[ID]'");
        redirect_js("index.php?m=relasi");
    }  
